      ...
      <table className="table table-bordered align-middle" id="milestone_achiever_table">
       ...
       <tbody>
            {rewardsData.map((rewards, index) => (
                ...
                <td>{rewards.teamMembers} <span className="normal-rewards-points" style={{color: Number(rewards.teamMembersMoreDetails1) >= "50000" ? "#1865D9" : "#FFA125"}}>{rewards.teamMembersMoreDetails1}</span></td>
                <td> <button className={`nominate_btn ${nominatedRows[index] ? "nominated" : ""}`} onClick={() => handleNominate(index)}>
                    ...
                    {approvedChampions[index] ? "CEO Approved" : "CEO Approval"}</button>
                </td>
                </tr>
            ))}
            </tbody>
...
export default PrideOfSteelTable;
